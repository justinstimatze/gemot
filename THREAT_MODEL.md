...
| Per-deliberation privacy boundary (MVP) | ✅ Implemented | Deliberations created with `visibility="private"` opt out of the global trust graph entirely: `service.Analyze` skips `reputation.UpdateFromRound` + `GetUnprocessedDisputes` on round close when the delib is private. Private-delib agreement patterns neither emit into `agent_trust_edges` nor increment `survived_count`, preventing leakage of confidential coordination into the globally-readable trust graph. `visibility="link"` is treated as public (discoverable-by-token, not consent-limited) so link-shared deliberations still participate in global reputation. Tradeoff documented: private delibs neither contribute to nor benefit from reputation weighting in the MVP; full per-delib private EigenTrust (partitioned edges via `deliberation_id` column + separate eigenvector) is tracked as a follow-up. Coverage: `tests/deliberation_privacy_test.go` exercises private-no-emit, public-still-emits, and link-still-emits invariants. |
...
| Per-delib private EigenTrust | Medium | MVP private boundary skips emission entirely — private delibs get no reputation benefit. Full version partitions `agent_trust_edges` by `deliberation_id`, computes per-delib EigenTrust from the private partition plus global edges, and keeps confidential agreement patterns out of the global graph while still providing Sybil defense within the private cohort. Schema change + Weigher refactor; tracked for a follow-up session. |
