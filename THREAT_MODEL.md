...
| Byzantine-tolerant sequence agreement | High | 🟡 Partially Implemented (session 2 of N). `internal/bft/` implements the chained-HotStuff core plus view change under Byzantine leader failure: proposal/vote/QC/two-chain commit, `Timeout()` broadcasting domain-0x03-signed NewView messages, `HandleNewView()` accumulating the 2f+1 quorum and selecting the highest-view highQC for the new leader's proposal. `specs/HotStuff.tla` + `HotStuff.cfg` model-check safety (no two honest replicas commit conflicting blocks at the same height; QC quorum threshold; NewView-carried QCs are real) in ~2s at N=4, f=1, and a stress cfg with view change exercises ~540k distinct states in ~10s. Adversarial test suite covers leader stall, cross-view equivocation, partitioned minority (no progress without quorum; resumes on heal), and stale-NewView (Byzantine-carried stale highQC loses selection to honest real highQC). Still deferred: real pure-Go BLS threshold sigs (gnark-crypto preliminary; no cgo per the Fly Dockerfile constraint), service-layer integration (routing `submit_position`/`vote`/`analyze` through the log), durable Postgres log replay, multi-node Fly deployment with `HTTPTransport` + real wall-clock timeout timer, client-side QC proof verification, formal temporal liveness property in TLA+. The current package is NOT wired into `internal/deliberation/service.go` — production requests still land on the single-server path. See `specs/hotstuff-design.md` for the full design + deferred list. (DARPA-PS-26-09 Track 1) |
