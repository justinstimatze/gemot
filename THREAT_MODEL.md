...
| Byzantine-tolerant sequence agreement | High | 🟡 Partially Implemented (session 1 of N). `internal/bft/` implements the chained-HotStuff core: proposal/vote/QC/two-chain commit under placeholder threshold signatures and an in-memory transport. `specs/HotStuff.tla` + `HotStuff.cfg` model-check the safety properties (no-two-honest-replicas-commit-conflicting-blocks-at-same-height, quorum threshold, commit-requires-QC) in ~1s at N=4, f=1. Deferred to session 2+: view change on leader timeout, real pure-Go BLS threshold sigs (gnark-crypto preliminary; no cgo per the Fly Dockerfile constraint), service-layer integration (routing `submit_position`/`vote`/`analyze` through the log), multi-node Fly deployment with `HTTPTransport`, client-side QC proof verification, Byzantine adversarial tests. The current package is NOT wired into `internal/deliberation/service.go` — production requests still land on the single-server path. See `specs/hotstuff-design.md` for the full design + deferred list. (DARPA-PS-26-09 Track 1) |
