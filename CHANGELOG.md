...
- **THREAT_MODEL.md updated**: "Edge-table pruning + cumulative-weight caps" and "Per-deliberation privacy boundary for trust edges" moved from planned → implemented; two new Planned rows added for subgraph-scoped `LoadTrustEdges` and full per-delib private EigenTrust.
...
- New `tests/deliberation_privacy_test.go` (3 Postgres tests, DARPA Track 1 Priority 4 MVP): private-delib full round closes cleanly with zero `agent_trust_edges` rows and zero `survived_count > 0` rows afterwards; public-delib regression ensures the default visibility still populates the trust graph; link-visibility delib emits edges (treated as public for reputation). Each test wires `reputation.NewWeigher` end-to-end, submits 4 positions, votes across all agents, and runs `Analyze` through a `repAnalyzer` that populates `SourcePositionIDs` + `AgreeAgents` so the edge-emission path is actually exercised.
