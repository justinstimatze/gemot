...
// taxonomyPrompt: %s = deliberation topic, %s = positions text
const taxonomyPrompt = `I will give you a list of positions from agents in a deliberation about "%s". I want you to propose a way to break down the information contained in these positions into topics and subtopics of interest.
...
// claimExtractionPrompt: %s = agent num, %s = deliberation topic, %s = taxonomy text, %s = position content
const claimExtractionPrompt = `I'm going to give you a position made by Participant %s in a deliberation about "%s", and a list of topics and subtopics which have already been extracted. I want you to extract the most important concise claims that the participant may support. We are only interested in claims that can be mapped to one of the given topic and subtopic. The claim must be fairly general but not a platitude. It must be something that other people may potentially disagree with. Each claim must also be atomic.
...
// claimDeduplicationPrompt: %s = deliberation topic, %s = subtopic name, %s = claims text
const claimDeduplicationPrompt = `You are grouping claims to help users understand which themes matter most in this deliberation about "%s", under the subtopic "%s". Your goal is to consolidate similar claims into well-supported groups while preserving genuinely unique perspectives.
...
// summaryPrompt: %s = deliberation topic, %s = topic name, %s = positions text
const summaryPrompt = `I'm going to give you a single topic from a deliberation about "%s" with positions from participants.

Generate a detailed summary (100-140 words) for the topic "%s" that:
...
// compromisePrompt: %s = deliberation topic, %s = cruxes text, %s = bridging text, %s = cluster descriptions
const compromisePrompt = `You are mediating a deliberation about "%s". Your task is to draft a compromise statement that the maximum number of participants could endorse.
...
// reframePrompt: %s = original position, %s = other positions summary, %s = cruxes summary
