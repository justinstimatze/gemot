		...
		"skills": []map[string]any{
			...
			},
			{
				...
				"description": "commit (with optional conditional commitments), get_commitments to list outstanding " +
					...
					"record_access logs that a downstream agent touched a commitment's artifact (read/question/dependency; a " +
					"dependency must cite dependent_commitment_id — your own commitment that relies on it — so stakes are " +
					"costly and attributable, not forgeable); get_signals returns the server-stamped, party-independent clock " +
					"and stakes marker derived from that ledger — an externally-visible disclosure window and consequence flag " +
					"neither party can backdate. " +
